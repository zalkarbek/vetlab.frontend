...
const rect = (el) =>
  el && Object.keys(el).length ? el.getBoundingClientRect() : null
  ...
  methods: {
    ...
    getRow(row) {
      switch (typeof this.showField) {
        case 'string':
          return row[this.showField] ? row[this.showField] : ''
        case 'function':
          return this.showField(row) ? this.showField(row) : ''
      }
    },
    showChange(val) {
      this.show = val
    ...
    populate() {
      ...
      if (!Array.isArray(this.data) || !this.data.length || !text) return []
    ...
    processControl(e) {
      if (![UP, DOWN, ESC, ENTER, TAB].includes(e.keyCode) || !this.list.length)
        return
      switch (e.keyCode) {
        case UP: // up
          this.previous()
    ...
    previous() {
      ...
      if (!this.show) this.open()
      this.highlight =
        this.highlight === -1 ? this.list.length - 1 : --this.highlight
      this.$nextTick(() => {
        const list = this.$refs.list
        const curPos = rect(list.querySelectorAll('.sg-over')[0])
        const listPos = rect(list)
